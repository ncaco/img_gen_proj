...
export default function CreatePage() {
  ...
  const cardPreviewRefMobile = useRef<HTMLDivElement>(null);
  ...
  const steps = {
    image: { completed: imageStepCompleted, enabled: true },
    info: { completed: infoStepCompleted, enabled: true },
    result: { completed: !!previewImageUrl, enabled: !!previewImageUrl },
  };
  ...
  const handleImageDrop = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      if (!characterImage) {
        ...
        if (currentStep === 'image') setTimeout(() => setCurrentStep('info'), 300);
  ...
  const generatePreviewImage = async (): Promise<string> => {
    ...
    try {
      const cardElement =
        (containerRef.querySelector('div[data-card-preview="true"]') as HTMLElement | null) ||
        containerRef;
  ...
  const uploadImage = async (
    imageData: string,
    imageType: 'character' | 'background'
  ): Promise<string | null> => {
    ...
    try {
      ...
      formDataUpload.append('subdirectory', 'cards');
  ...
  const handleGenerate = async () => {
    try {
      ...
      const characterImageUrl = characterImage ? await uploadImage(characterImage, 'character') : null;
  ...
  const cardData = {
    ...
    attack: formData.attack,
    health: formData.health,
    skill1: { name: formData.skill1Name, description: formData.skill1Description },
    skill2: { name: formData.skill2Name, description: formData.skill2Description },
    flavorText: formData.flavorText,
    cardNumber: formData.cardNumber,
    series: formData.series,
  };
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center flex-1">
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">카드 생성기</h1>
            <p className="text-gray-600 dark:text-gray-400">단계별로 카드를 생성하세요</p>
          </div>
          <div className="w-20" />
        </div>
